...
dhtmlXTreeObject.prototype.findXML = function(node, par, val) {
    ...
    for (var i = 0; i < node.childNodes.length; i++)
        ...
        {
            //if (confirm(node.childNodes[i].getAttribute(par)+"="+val+"||"+node.childNodes[i].getAttribute("type")+"="+val2)) return dfasf.asdfasd();
...
dhtmlXTreeObject.prototype.setItemStyle = function(itemId, style_string) {
    ...
    if (!temp) return 0;
    /*if (!_isOpera)
     (temp.span.runtimeStyle||temp.span.style).cssText += style_string;
     else
     this.obj.setAttribute("style",objCssText);
     */
...
dhtmlXTreeObject.prototype._parseXMLTree = function(dhtmlObject, node, parentId, level, xml_obj, start) {
    ...
    if (dhtmlObject.parsCount == 1) {
        ...
        {
            //temporary fix for Opera, must be removed after Opera 9.0 will be launched
            if (!_isOpera)
    ...
    if (dhtmlObject._edsbps) window.setTimeout(function() {
        dhtmlObject._distributedStep(parentId);
    }, dhtmlObject._edsbpsD);
 ...
 *     @topic: 6
 *
 *     增加图片路径对上下文"\",和"http:\\"的支持.
 *     alert by wenjg 2009-12-02
 */
dhtmlXTreeObject.prototype._correctPlus = function(itemObject) {
    ...
    if ((this.XMLsource) && (!itemObject.XMLload))
    {
        ...
        imsrc2.src = this._setImagePath(itemObject.images[2]);
        ...
        if ((itemObject.childsCount) || (itemObject.unParsed))
        {
            if ((itemObject.htmlNode.childNodes[0].childNodes[1]) && ( itemObject.htmlNode.childNodes[0].childNodes[1].style.display != "none" ))
            {
                ...
                imsrc2.src =this._setImagePath(itemObject.images[1]);
                if (this._txtimg) return (imsrc.innerHTML = "[-]");
            }
            else
            {
        ...
        {
            imsrc2.src = this._setImagePath(itemObject.images[0]);
        }
...
/**
 * 设置上下文路径图片, appResourceServerUrl为平台的资源目录.
 * add by wenjg 2009-12-02
 *
 * @param image
 */
dhtmlXTreeObject.prototype._setImagePath = function(image) {
    var correntImagePath;
    if (image.indexOf("/") == 0) {
        correntImagePath = appResourceServerUrl + image;
    }else if (image.indexOf("http://") == 0) {
        correntImagePath = image;
    } else {
        correntImagePath = this.imPath + image;
    }

    return correntImagePath;
}
...
dhtmlXTreeObject.prototype.onRowClick2 = function() {
    if (this.parentObject.treeNod.dblclickFuncHandler)
        if (!this.parentObject.treeNod.dblclickFuncHandler(this.parentObject.id, this.parentObject.treeNod))
            return 0;
...
dhtmlXTreeObject.prototype.setOnOpenHandler = function(func) {
    if (typeof(func) == "function") this._spnFH = func; else this._spnFH = eval(func);
};
/**
 *     @desc: set function called before tree node opened/closed
 *     @param: func - event handling function
...
dhtmlXTreeObject.prototype.setOnOpenEndHandler = function(func) {
    if (typeof(func) == "function") this._epnFH = func; else this._epnFH = eval(func);
};
 ...
 *     @topic: 4
 */
dhtmlXTreeObject.prototype.getObjItemId = function(itemId)
{
    alert(itemId);
...
dhtmlXTreeObject.prototype.enableHighlighting = function(mode) {
    this.ehlt = convertStringToBoolean(mode);
};
...
dhtmlXTreeObject.prototype.setDropHandler = function(func) {
    if (typeof(func) == "function") this.dropFunc = func; else this.dropFunc = eval(func);
};
 ...
 *     @type:  public
 *     @param: itemId - item Id
 *     @param: mode - moving mode (left,up,down,item_child,item_sibling)
...
dhtmlXTreeObject.prototype.setOnLoadingStart = function(func) {
    if (typeof(func) == "function") this.onXLS = func; else this.onXLS = eval(func);
};
/**
...
dhtmlXTreeObject.prototype.setOnLoadingEnd = function(func) {
    if (typeof(func) == "function") this.onXLE = func; else this.onXLE = eval(func);
};
...
dhtmlXTreeObject.prototype._loadDynXML = function(id) {
    //load userdata
    var arr = this.getAllUserData(id);
    var urlParam = "";
    if (arr != null)
    {
    ...
    if (this.xmlalb == "function") {
        ...
        if (this.xmlalb == "name")
            ...
            if (this.xmlalb == "xmlname")
                this.loadXML(this.XMLsource + this._escape(id) + ".xml?uid=" + sn + urlParam);
            else
                this.loadXML(this.XMLsource + getUrlSymbol(this.XMLsource) + "uid=" + sn + "&tree_node_id=" + this._escape(id) + urlParam + "&attach_param=" + this.attachParam);
};
...
dhtmlXTreeObject.prototype._distributedStep = function(pId) {
    var self = this;
    /*
     if ((this.XMLloadingWarning)&&(this.onXLE)){
...
dhtmlXTreeObject.prototype.getAllUserData = function(itemId)
{
    var sNode = this._globalIdStorageFind(itemId, 0, true);
    if (!sNode || !sNode._userdatalist) return;
    var ids = sNode._userdatalist.split(",")
    var value = "";
    var arr = new Array();
    for (var i = 0; i < ids.length; i++)
    {
        value = sNode.userData["t_" + ids[i]];
...
dhtmlXTreeObject.prototype.getItemById = function(id)
{
    var item = null;
    for (var i = 0; i < this._selected.length; i++)
    {
